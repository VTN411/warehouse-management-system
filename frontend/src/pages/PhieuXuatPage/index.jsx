...
const PERM_EDIT_APPROVED = 121; // Quyền sửa phiếu xuất đã duyệt
...
const PhieuXuatPage = () => {
  ...
  const canEditApproved = checkPerm(PERM_EDIT_APPROVED); // [!] Check quyền mới
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const allowEdit = isEditable(record); // [!] Kiểm tra điều kiện hiển thị nút Sửa
        ...
        return (
          ...
          >
            ...
            {/* [!] HIỂN THỊ NÚT SỬA DỰA TRÊN ĐIỀU KIỆN */}
            {allowEdit && (
              <Button
                icon={<EditOutlined />}
                          ...
                          validator(_, value) {
                            ...
                            if (inStock && value > inStock.soLuongTon) {
                              return Promise.reject(
                                new Error(`Quá tồn kho (${inStock.soLuongTon})`)
                              );
                            }
              ...
              columns={[
                {
                  ...
                  key: "maSP",
                  render: (id) =>
                    listSanPham.find((sp) => sp.maSP === id)?.tenSP ||
                    `SP-${id}`,
                },
                {
                  title: "Số Lượng",
