...
const ReportPage = () => {
  ...
  const inventoryColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (current <= 0) return <Tag icon={<WarningOutlined />} color="red">Hết hàng</Tag>;
        if (current <= min) return <Tag icon={<WarningOutlined />} color="orange">Sắp hết</Tag>;
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      {items.length > 0 ? (
        <Tabs defaultActiveKey={items[0].key} items={items} onChange={handleTabChange} />
      ) : (
